...
void draw() {
  ...
  if (menu == 0) {
    ...
    if (gen == -1) {
      ...
      text("Do gens ALAP[" + threads + "].", 1000, 150);
  ...
  if (menu == 6) {
    ...
    //new
    java.util.Collections.sort(c2, new java.util.Comparator<Creature>() {
            @Override
            public int compare(Creature o1, Creature o2) {
                return Float.compare(o2.d, o1.d);
            }
        });
    //endnew
...
void keyPressed(){
  ...
  if(key == 'h') threads = Math.max(1,threads-1);
  if(key == 'y') threads++;
